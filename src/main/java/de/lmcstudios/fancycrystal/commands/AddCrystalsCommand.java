...
public class AddCrystalsCommand implements CommandExecutor {

    private final FancyCrystalPlugin plugin;
    private static final MiniMessage MM = MiniMessage.miniMessage();

    public AddCrystalsCommand(FancyCrystalPlugin plugin) { this.plugin = plugin; }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (args.length < 2) {
            sender.sendMessage(MM.deserialize(prefix + "<red>Nutzung: /addcrystals <Spieler> <Betrag>"));
            return true;
        }
