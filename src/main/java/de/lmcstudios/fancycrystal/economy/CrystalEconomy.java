...
public class CrystalEconomy implements Economy {
    ...
    @Override public String getName() { return "Crystals"; }
    @Override public boolean hasBankSupport() { return false; }
    @Override public int fractionalDigits() { return plugin.getConfig().getInt("currency.decimals", 2); }
    @Override public String format(double amount) { return NumberFormatter.formatWithSymbol(amount); }
    @Override public String currencyNamePlural() { return plugin.getConfig().getString("currency.name-plural", "Crystals"); }
    @Override public String currencyNameSingular() { return plugin.getConfig().getString("currency.name", "Crystal"); }
    ...
    @Override public double getBalance(String playerName) {
        return storage.getBalance(plugin.getServer().getOfflinePlayer(playerName).getUniqueId());
    }
    @Override public double getBalance(String playerName, String world) { return getBalance(playerName); }
    @Override public double getBalance(OfflinePlayer player) { return storage.getBalance(player.getUniqueId()); }
    ...
    @Override public EconomyResponse withdrawPlayer(OfflinePlayer player, double amount) {
        if (amount < 0) return new EconomyResponse(0, 0, EconomyResponse.ResponseType.FAILURE, "Betrag negativ");
        UUID uuid = player.getUniqueId();
        double bal = storage.getBalance(uuid);
        if (bal < amount) return new EconomyResponse(0, bal, EconomyResponse.ResponseType.FAILURE, "Nicht genug Guthaben");
